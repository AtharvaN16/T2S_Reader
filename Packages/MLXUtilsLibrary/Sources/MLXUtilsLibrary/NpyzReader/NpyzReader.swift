...
public final class NpyzReader {
  ...
  /// Patched for the vendored copy: reads through `NpzArchive` rather than ZIPFoundation, with
  /// the same contract — file entries by name, `nil` when the archive cannot be read.
  private static func unarchive(data: Data) -> [String: Data]? {
    do {
      return try NpzArchive.entries(in: data)
    } catch {
