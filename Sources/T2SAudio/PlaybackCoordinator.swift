...
public final class PlaybackCoordinator {
    ...
    private func fill() async {
        ...
        // Re-reads `self.timeline` fresh every iteration (rather than a `guard let` snapshot taken
        // once): the `await store.read` below is a real suspension point, during which a concurrently
        // processed `.rendered` event can update `self.timeline` with a newly-available audioRef —
        // a stale local copy would keep reporting that utterance as unrendered.
        while let timeline, next < timeline.utteranceCount, queuedCount < configuration.queuedSegments {
